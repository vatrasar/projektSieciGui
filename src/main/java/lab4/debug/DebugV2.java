...
public class DebugV2 {
    ...
    public void addSixEvolutionary(List<Sensor> sensorsList) {
        String[] headerLine = new String[1];
        headerLine[0] = "#Debug part 6b";
        lines.add(headerLine);
        for (var sensor : sensorsList) {

            //header for sensor neighbours
            lines.add(Utils.convertForArray("#Sensor id",sensor.getIdentyfikator()+""));
            lines.add(Utils.convertForArray("#Sensor current Strategy",sensor.getLastStrategy().getName()));
            lines.add(Utils.convertForArray("#Sensor neighbours"));
            String[] neighbourHeader = new String[3];
            neighbourHeader[0] = "#id";
            neighbourHeader[1] = "Strategy";
            neighbourHeader[2] = "probablility range";
            lines.add(neighbourHeader);
            if(sensor.hasAllNeighboursAndSensorHasZeroSumU()){
                double rangeStart = 0;
                double sensorNumber=sensor.neighborSensors.size()+1;
                String[] sensorRecord = new String[3];
                sensorRecord[0] = sensor.getIdentyfikator() + "";
                sensorRecord[1] =  sensor.getLastStrategy().getName();
                sensorRecord[2] = "(" + Utils.stringFormater(rangeStart) + "|" + Utils.stringFormater(rangeStart + 1/sensorNumber) + ")";
                rangeStart=rangeStart +  1/sensorNumber;
                lines.add(sensorRecord);
                for (var neighbour : sensor.neighborSensors) {

                    String[] neighbourRecord = new String[3];
                    neighbourRecord[0] = neighbour.getIdentyfikator() + "";
                    neighbourRecord[1] = neighbour.getLastStrategy().getName();
                    neighbourRecord[2] = "(" + Utils.stringFormater(rangeStart) + "|" + Utils.stringFormater(rangeStart +1/sensorNumber) + ")";
                    rangeStart=rangeStart + 1/sensorNumber;
                    lines.add(neighbourRecord);
                }
                lines.add(Utils.convertForArray("random x",Utils.stringFormater(sensor.getRandomSenorX())));
            }
            else {
                double rangeStart = 0;
                double uRewardSum=sensor.getNeighborsSumU()+sensor.getSum_u();
                String[] sensorRecord = new String[3];
                sensorRecord[0] = sensor.getIdentyfikator() + "";
                sensorRecord[1] =  sensor.getLastStrategy().getName();
                sensorRecord[2] = "(" + Utils.stringFormater(rangeStart) + "|" + Utils.stringFormater(rangeStart + sensor.getSum_u()/uRewardSum) + ")";
                rangeStart=rangeStart +  sensor.getSum_u()/uRewardSum;
                lines.add(sensorRecord);
                for (var neighbour : sensor.neighborSensors) {
                    ...
                    neighbourRecord[0] = neighbour.getIdentyfikator() + "";
                    neighbourRecord[1] = neighbour.getLastStrategy().getName();
                    neighbourRecord[2] = "(" + Utils.stringFormater(rangeStart) + "|" + Utils.stringFormater(rangeStart + neighbour.getSum_u()/uRewardSum) + ")";
                    rangeStart=rangeStart + neighbour.getSum_u()/uRewardSum;
                    lines.add(neighbourRecord);
                }
                lines.add(Utils.convertForArray("random x",Utils.stringFormater(sensor.getRandomSenorX())));
            }
